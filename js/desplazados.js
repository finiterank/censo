...
var g = d3.select("#diagrama").append("svg")
        .attr("width", width)
        .attr("height", height)
    .append("g")
        .attr("id", "circle")
...
var m, transpuesta, sumasdesplazados, departamentos

function ready(error, matriz, dept){

    departamentos = d3.csv.parseRows(dept)[0];

    m = d3.csv.parseRows(matriz).map(function(row){
        return row.map(function(value) {
            return +value;
        });
    });

    transpuesta =  d3.transpose(m);

    sumasdesplazados = transpuesta.map(function(d){
...
function updateChords(matrix, modo) {
    ...
    newGroups.append("path")
        ...
        .style("fill", function (d) {
            return color(departamentos[d.index]);
        });
    ...
    chordPaths.select("title")
        .text(function(d) {
            if (departamentos[d.target.index] !== departamentos[d.source.index]) {
                if (modo === 0){
    ...
    chordPaths.transition()
        ...
        .style("fill", function (d) {
            if (modo === 0){
                ...
                return color(departamentos[d.target.index]);
            }
        })
...
d3.select("#HastaButton").on("click", function () {
    updateChords(transpuesta, 1);
...
d3.select("#DesdeButton").on("click", function() {
    updateChords(m, 0);
    disableButton(this);
});
